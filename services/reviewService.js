const { default: Reviews } = require('../../client/src/components/Comments/Reviews');
...
class ReviewService {
	async addReview(userId, deviceId, userRate, userComment) {
		...
		if (!isRateExist) {
			const addedReview = await Review.create({ userId, deviceId, rate: userRate, comment: userComment });
			const device = await deviceService.updateReview(deviceId);
			return device;
		}

		const addedReview = await Review.update({ rate: userRate, comment: userComment }, { where: { userId, deviceId } });
	...
	async confirmReview(userId, deviceId, confirmed) {
		...
		const confirmedReview = await Review.update({ confirmed: confirmed }, { where: { userId, deviceId } });
